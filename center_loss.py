...
class FCOSLoss(nn.Module):
    ...
    def computation_targets(self,points,targets,revise):
        ...
        labels,reg_target=self.computate_point_for_location(points_all,targets,expanded_siz,revise)
    ...
    def computate_point_for_location(self,locations,targets,target_of_areas,revise):
        xs=locations[:,0]
        ys=locations[:,1]
        N=len(targets)
        labelss,regs=[],[]
        for i in range(N):
            ...
            rev=revise[i].view(-1,2)
            ###revise location(center point) by semantic center
            xs=xs+rev[:,0]
            ys=ys+rev[:,1]
    ...
    ###
    def forward(self,locations,cls,box,centerness,revise,targets):
        batch,num_classes,h,w=cls[0].shape
        
        label,reg_target=self.computation_targets(locations,targets,revise)
        box_label_flatten=[]
